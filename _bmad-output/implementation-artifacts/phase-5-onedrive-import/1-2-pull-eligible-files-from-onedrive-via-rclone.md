...
- [ ] **Task 1: Build the rclone copy invocation** (AC: #1)
  - [ ] In `rclone.py` (or a small `pull` helper), add a function that returns the argument vector: `["copy", "onedrive-monkai:PUBLIC-DATA/LIBERET/BOOK-FILES", "./staging/onedrive/", "--include", "*.epub", "--include", "__books.json"]`. Keep arg construction pure/testable; execution separate.
  - [ ] Ensure `staging/onedrive/` is created (mkdir parents) before invoking rclone.
- [ ] **Task 2: Implement `pull` command** (AC: #1, #2, #3)
  - [ ] Wire `sync.py pull` to call the rclone wrapper with the vector above; stream/surface rclone output to the console (operator needs to see the SSO prompt and progress).
  - [ ] On non-zero exit, let `rclone.py`'s `RuntimeError` propagate with stderr — fail fast.
  - [ ] Do NOT add any token/credential handling — rclone owns OAuth in `~/.config/rclone/rclone.conf`.
- [ ] **Task 3: Tests** (AC: #1, #2)
  - [ ] `tests/test_sync.py` (or `test_pull.py`): assert the rclone argument vector matches AC #1 exactly (mock the executor — no live rclone).
  - [ ] Assert that a mocked non-zero rclone exit raises and surfaces stderr.
- [ ] **Task 4: Verify** (AC: #4)
  - [ ] Confirm `.gitignore` ignores `staging/`; `git status` shows no staged epub/manifest after a (real or simulated) pull.
